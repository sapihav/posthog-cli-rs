...
- **Hard max:** 500 lines of source code (in `src/`, excluding tests, generated code, vendored deps, and trivial init files)
...
- Verify: `cargo build` succeeds, `cargo run -- --help` works, `cargo run -- schema` parses as JSON, `cargo test` passes, no regressions in existing commands
